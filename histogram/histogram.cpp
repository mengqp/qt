...
Histogram::Histogram( QWidget *p)  : QWidget(p)
{
	...
	customPlot->setGeometry( 0, 0 , 800, 480 );
	...
	for (int i = 0; i < x1.size(); ++i) {
		...
		y1[i] =
			qCos(x1[i] * 0.8 + qSin(x1[i] * 0.16 + 1.0)) * qSin(x1[i] * 0.54) +
			1.4;
	}
	for (int i = 0; i < x2.size(); ++i) {
		x2[i] = i / (double)(x2.size() - 1) * 10;
		y2[i] = qCos(x2[i] * 0.85 + qSin(x2[i] * 0.165 + 1.1)) *
					qSin(x2[i] * 0.50) +
				1.7;
	}
	...
	bars2->setBrush(QColor(10, 140, 70, 160));
